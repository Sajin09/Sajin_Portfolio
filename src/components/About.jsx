      ...
      <div className="about-container">
        ...
        <div className="about-desktop-wrapper">
          ...
          <div className="about-bento-grid">
            ...
            {/* Right Column: 3D Coverflow Perspective Carousel */}
        ...
        {/* =========================================================================
           2. MODERN ALTERNATIVE MOBILE ABOUT VIEW (Visible ONLY on <= 900px)
           ========================================================================= */}
        <div className="about-mobile-layout">
          
          {/* Mobile Header */}
          <div className="about-mob-header">
            <span className="about-mob-script">Professional Overview</span>
            <h2 className="about-mob-title">
              <span>ABOUT</span>
              <span className="about-mob-title-gradient">SAJIN R.</span>
            </h2>
            <div className="about-mob-pill">
              <span className="mob-badge-dot"></span>
              <span>Full Stack &amp; AI Specialist</span>
            </div>
          </div>

          {/* Mobile Marquee Ticker */}
          <div className="about-mob-marquee">
            <div className="mob-marquee-track">
              {techHighlights.slice(0, 8).map((tech, idx) => (
                <span key={idx} className="mob-marquee-item">
                  <span>{tech}</span>
                  <span className="mob-marquee-star">✦</span>
                </span>
              ))}
              {techHighlights.slice(0, 8).map((tech, idx) => (
                <span key={`dup-${idx}`} className="mob-marquee-item">
                  <span>{tech}</span>
                  <span className="mob-marquee-star">✦</span>
                </span>
              ))}
          ...
          {/* Mobile Profile Story Card */}
          <div className="about-mob-story-card">
            <h3 className="mob-story-heading">
              BUILDING ENTERPRISE MERN APPLICATIONS &amp; AI SYSTEMS
            </h3>
            
            <p className="mob-story-text">
              Full-Stack Developer with <strong>3+ years</strong> of hands-on experience delivering robust web applications using React.js, Node.js, Express.js, MongoDB, and Firebase. Specializing in secure REST APIs, scalable architectures, and next-gen AI automation workflows.
            </p>

            {/* Quick Info Matrix */}
            <div className="mob-quick-info-grid">
              <div className="mob-info-chip">
                <MapPin size={15} className="mob-chip-icon" />
                <span>Nagercoil, Tamil Nadu (Open to Relocation)</span>
              </div>
              <div className="mob-info-chip">
                <Briefcase size={15} className="mob-chip-icon" />
                <span>MERN Developer @ Rooks &amp; Brooks Tech</span>
              </div>
              <div className="mob-info-chip">
                <GraduationCap size={15} className="mob-chip-icon" />
                <span>B.E. ECE • Anna University (8.1 CGPA)</span>
              </div>
            </div>
          </div>

          {/* Mobile Interactive Pillar Capabilities */}
          <div className="about-mob-pillars-section">
            <div className="mob-pillars-header">
              <span className="mob-pillars-title">CORE CAPABILITIES</span>
              <span className="mob-pillars-indicator">TAP TO EXPLORE</span>
            </div>

            {/* Tab Bar Selector */}
            <div className="mob-pillar-tabs">
              {cards.map((card, idx) => (
                <button
                  key={card.id}
                  className={`mob-pillar-tab-btn ${idx === activeIndex ? 'active' : ''}`}
                  onClick={() => setActiveIndex(idx)}
                >
                  <span>{card.tabLabel}</span>
                  {idx === activeIndex && <span className="tab-active-indicator"></span>}
                </button>
              ))}
            </div>

            {/* Active Pillar Card */}
            <div key={currentCard.id} className="mob-active-pillar-card">
              <div className="mob-pillar-top">
                <div className="mob-pillar-stat-group">
                  <span className="mob-pillar-stat">{currentCard.stat}</span>
                  <span className="mob-pillar-title">{currentCard.title}</span>
                </div>
                <div className="mob-pillar-icon-box">
                  {currentCard.icon}
                </div>
              </div>

              <p className="mob-pillar-desc">{currentCard.desc}</p>

              <div className="mob-pillar-highlights">
                {currentCard.highlights.map((item, hIdx) => (
                  <div key={hIdx} className="mob-pillar-highlight-row">
                    <span className="mob-highlight-bullet">✦</span>
                    <span className="mob-highlight-text">{item}</span>
                  </div>
                ))}
              </div>

              <div className="mob-pillar-footer">
                <div className="mob-pillar-status">
                  <span 
                    className="mob-status-dot-pulse"
                    style={{ 
                      backgroundColor: currentCard.statusDotColor,
                      boxShadow: `0 0 8px ${currentCard.statusDotColor}` 
                    }}
                  />
                  <span>{currentCard.statusText}</span>
                </div>
                <span className="mob-pillar-badge">{currentCard.badgeText}</span>
              </div>
            </div>

            {/* Navigation Arrows & Dots */}
            <div className="mob-pillar-controls">
              <button 
                className="mob-arrow-btn" 
                onClick={prevSlide}
              ...
              <div className="mob-carousel-dots">
                {cards.map((_, dotIdx) => (
                  <button
                    key={dotIdx}
                    className={`mob-dot ${dotIdx === activeIndex ? 'active' : ''}`}
                    onClick={() => setActiveIndex(dotIdx)}
                    aria-label={`Go to slide ${dotIdx + 1}`}
                  />
                ))}
          ...
          <div className="about-mob-actions">
            <a href="#projects" className="mob-action-btn primary">
              <span>View MERN Projects</span>
              <ArrowRight size={16} />
            </a>
            <a 
              ...
              target="_blank"
              rel="noreferrer"
              className="mob-action-btn outline"
            >
              <Mail size={16} />
              <span>sajin0904@gmail.com</span>
            </a>
